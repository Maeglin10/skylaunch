...
import { X, Menu, Search, Droplets, Zap, Activity, Globe, Shield, Command, Plus, ArrowUpRight, Maximize2, MoveRight, Layers, Box, Compass, Sparkles, MoveVertical, Waves } from "lucide-react";
import "../premium.css";

const SLIDES = [
  { id: 1, title: "COBALT_SYNC", cat: "Fluid", value: "Locked", img: "https://images.unsplash.com/photo-1550684848-fac1c5b4e853?q=80&w=1000&auto=format&fit=crop" },
  { id: 2, title: "AMBER_NODE", cat: "Static", value: "Active", img: "https://images.unsplash.com/photo-1541701494587-cb58502866ab?q=80&w=1000&auto=format&fit=crop" },
  { id: 3, title: "ROSE_PULSE", cat: "Dynamic", value: "Verified", img: "https://images.unsplash.com/photo-1506126613408-eca07ce68773?q=80&w=1000&auto=format&fit=crop" },
];

export default function LiquidFluxSPA() {
  const [view, setView] = useState<"flux" | "vector" | "mapping">("flux");
    ...
    <div className="premium-theme bg-[#050505] text-[#f8f8f8] min-h-screen selection:bg-white selection:text-black font-mono overflow-x-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-5">
           <div className="text-[60vw] font-black italic tracking-tighter select-none">FLUX</div>
        </div>
      </div>
      ...
      <nav className="fixed top-0 left-0 w-full z-50 p-8 md:p-12 flex justify-between items-center bg-black/40 backdrop-blur-3xl border-b border-white/10">
        <div className="flex gap-12 items-center">
           <button onClick={() => setView("flux")} className="text-xl font-black italic tracking-tighter hover:text-white transition-colors flex items-center gap-4">
              <Waves className="w-6 h-6 animate-pulse" /> FLUX_OS&trade;
           </button>
           <div className="hidden lg:flex gap-8 text-[10px] font-black uppercase tracking-widest opacity-20 italic">
              ...
              <span className="text-white">Ref: 0x93_L</span>
           </div>
        </div>
        <div className="hidden md:flex gap-12 text-[10px] font-black uppercase tracking-[0.4em] opacity-30">
           <button onClick={() => setView("flux")} className={`hover:opacity-100 transition-opacity ${view === 'flux' ? 'text-white opacity-100 underline decoration-white underline-offset-8 italic' : ''}`}>THE_FLUX</button>
      ...
      <AnimatePresence mode="wait">
        ...
        {/* THE FLUX VIEW (LANDING) */}
        {view === "flux" && (
          <motion.div key="flux" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pt-48 pb-32 px-12 max-w-[1800px] mx-auto min-h-screen flex flex-col justify-center relative z-10">
             <header className="mb-24 border-b-2 border-white/20 pb-12 flex flex-col md:flex-row justify-between items-end gap-12">
                <div>
                   <span className="text-[10px] uppercase font-black tracking-[1em] opacity-40 mb-4 block underline decoration-white/10 underline-offset-8 italic">Chromatic_Sync // Series_093</span>
                   <h1 className="text-7xl md:text-[12vw] font-black italic uppercase tracking-tighter leading-[0.75] text-white">LIQUID. <br/> <span className="text-transparent" style={{ WebkitTextStroke: '2px white' }}>PHASE.</span></h1>
                </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {SLIDES.map((p, i) => (
                  <motion.div 
                    ...
                    onClick={() => { setActiveItem(i); setView("vector"); }}
                  >
        ...
        {/* THE VECTOR VIEW (DETAIL) */}
        {view === "vector" && (
          <motion.div key="vector" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative z-10 min-h-screen">
             <button onClick={() => setView("flux")} className="fixed top-12 left-12 z-[60] bg-white text-black p-5 rounded-full hover:scale-110 transition-transform shadow-2xl">
                <X className="w-6 h-6" />
             </button>
                   ...
                   <div className="max-w-[1500px] w-full grid grid-cols-1 lg:grid-cols-2 gap-24 items-center relative z-10">
                      ...
                      <div className="flex flex-col justify-center space-y-12">
                         <div className="space-y-6">
                            <span className="text-[10px] uppercase tracking-[1em] font-black opacity-40 mb-8 block underline decoration-white decoration-4 underline-offset-8 italic">Phased_Sync // {SLIDES[activeItem].cat}</span>
                         ...
                         <p className="text-3xl font-light italic leading-relaxed uppercase tracking-tight opacity-40 text-white leading-relaxed">
                            Structural allocation for {SLIDES[activeItem].title}. System integrity at 100%. Thermal load nominal at 32C. Every coordinate synchronized.
                         </p>
                         ...
                         <div className="flex gap-6 pt-8">
                            <button onClick={() => setView("flux")} className="flex-grow py-8 bg-white text-black font-black uppercase text-xs tracking-[1em] hover:bg-white/80 transition-all shadow-2xl">
                               Return_to_Flux
                            </button>
          ...
          <motion.div key="mapping" initial={{ opacity: 0, scale: 1.1 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="relative z-10 pt-48 pb-32 px-12 max-w-7xl mx-auto min-h-screen flex flex-col justify-center">
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-32 items-center text-white">
                <div className="space-y-16">
                   <span className="text-[10px] uppercase font-black tracking-[1.5em] opacity-30 block underline decoration-white decoration-2 underline-offset-8 italic">The_Mapping_Protocol</span>
