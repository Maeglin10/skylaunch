...
import { X, Menu, Search, Award, Zap, Activity, Globe, Shield, Command, Plus, ArrowUpRight, Maximize2, MoveRight, Layers, Box, Compass, Sparkles, MoveVertical, Target, Radio, CheckCircle2, Home, Layout, Landmark } from "lucide-react";
import "../premium.css";

const PROJECTS = [
  { id: 1, title: "CORE_MUSEUM", cat: "Culture", value: "Verified", img: "https://images.unsplash.com/photo-1518005020951-eccb494ad742?q=80&w=1000&auto=format&fit=crop" },
  { id: 2, title: "VOID_PAVILION", cat: "Public", value: "Active", img: "https://images.unsplash.com/photo-1503387762-592da58efbb3?q=80&w=1000&auto=format&fit=crop" },
  { id: 3, title: "NEON_HORIZON", cat: "Luxury", value: "Locked", img: "https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=1000&auto=format&fit=crop" },
];

export default function VoidArchSPA() {
  const [view, setView] = useState<"void" | "form" | "logic">("void");
  ...
  return (
    <div className="premium-theme bg-white text-black min-h-screen selection:bg-black selection:text-white font-sans overflow-x-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        ...
        <div className="absolute inset-x-0 bottom-0 h-1/2 bg-[radial-gradient(circle_at_center,_transparent_0%,_#ffffff_100%)] opacity-80" />
      </div>
      ...
      <nav className="fixed top-0 left-0 w-full z-50 p-8 md:p-12 flex justify-between items-center bg-transparent backdrop-blur-3xl border-b border-black/10 font-mono text-black">
        <div className="flex gap-12 items-center">
           <button onClick={() => setView("void")} className="text-xl font-black italic tracking-tighter hover:text-stone-800 transition-colors flex items-center gap-4 text-black">
              VOID_ARCH&trade;
           </button>
           <div className="hidden lg:flex gap-8 text-[10px] font-black uppercase tracking-widest opacity-20 italic">
              Status: Archive_Sync_Active
              <span className="text-black">Ref: 0x147</span>
           </div>
        </div>
        <div className="hidden md:flex gap-12 text-[10px] font-black uppercase tracking-[0.4em] opacity-30">
           <button onClick={() => setView("void")} className={`hover:opacity-100 transition-opacity ${view === 'void' ? 'text-black opacity-100 underline decoration-black underline-offset-8 italic' : ''}`}>THE_ARCHIVE</button>
      ...
      <AnimatePresence mode="wait">
        ...
        {view === "void" && (
          <motion.div key="void" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pt-48 pb-32 px-12 max-w-[1800px] mx-auto min-h-screen flex flex-col justify-center relative z-10 font-sans">
             <header className="mb-24 border-b border-black/20 pb-12 flex flex-col md:flex-row justify-between items-end gap-12 text-black">
                <div>
                   <span className="text-[10px] uppercase font-black tracking-[1em] opacity-40 mb-4 block underline decoration-black/10 underline-offset-8 italic font-mono text-black">Visual_Capture // Series_147</span>
                   <h1 className="text-7xl md:text-[12vw] font-black italic uppercase tracking-tighter leading-[0.75]">PURE. <br/> <span className="text-transparent" style={{ WebkitTextStroke: "2px rgba(0,0,0,0.6)" }}>FORM.</span></h1>
                </div>
                <div className="text-right flex flex-col items-end">
                   <div className="text-3xl font-black mb-4 tracking-tighter uppercase opacity-10 italic font-mono text-[#1a1a1a]">Structural_Sync</div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-black font-sans">
                {PROJECTS.map((p, i) => (
                  <motion.div 
                  ...
                  >
                     ...
                     <div className="absolute inset-10 flex flex-col justify-between">
                        <div className="flex justify-between items-start text-white">
                           ...
                           <div className="text-[10px] font-black uppercase tracking-widest opacity-20 italic">UNIT_0x{i+147}</div>
                        </div>
        ...
        {view === "form" && (
          <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative z-10 min-h-screen">
             <button onClick={() => setView("void")} className="fixed top-12 left-12 z-[60] bg-black text-white p-5 rounded-none hover:scale-110 transition-transform shadow-2xl">
                <X className="w-6 h-6" />
             </button>
             ...
             <div className="grid grid-cols-1 lg:grid-cols-12 min-h-screen pt-24 lg:pt-0">
                <div className="lg:col-span-12 relative flex items-center justify-center p-8 md:p-32 overflow-hidden h-screen bg-white">
                   ...
                   <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_transparent_0%,_#ffffff_100%)]" />
                   ...
                   <div className="max-w-[1500px] w-full grid grid-cols-1 lg:grid-cols-2 gap-24 items-center relative z-10 font-sans text-black">
                      ...
                      <div className="flex flex-col justify-center space-y-12 text-black">
                         <div className="space-y-6">
                            <span className="text-[10px] uppercase tracking-[1em] font-black opacity-30 mb-8 block underline decoration-black decoration-4 underline-offset-8 italic font-mono text-black">Dynamic_Sync // {PROJECTS[activeItem].cat}</span>
                            <h1 className="text-7xl md:text-[8vw] font-black italic uppercase tracking-tighter leading-none text-black">{PROJECTS[activeItem].title}</h1>
                         ...
                         <div className="grid grid-cols-2 gap-12 py-12 border-y border-black/10 font-mono text-black/60 text-black">
                            {[
                              { icon: <Globe className="w-5 h-5" />, l: "Logic", v: "Phase_Shift" },
                         ...
                         <div className="flex gap-6 pt-8 font-mono">
                            <button onClick={() => setView("void")} className="flex-grow py-8 bg-black text-white font-black uppercase text-xs tracking-[1em] hover:bg-stone-800 transition-all shadow-2xl">
                               Return_to_Archive
                            </button>
                            <button className="px-12 py-8 border border-black/10 text-[10px] font-black uppercase tracking-[0.5em] hover:scale-105 transition-all text-black">
                               PDF_Spec
                            </button>
                         </div>
                      </div>
        ...
        {view === "logic" && (
          <motion.div key="logic" initial={{ opacity: 0, scale: 1.1 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="relative z-10 pt-48 pb-32 px-12 max-w-7xl mx-auto min-h-screen flex flex-col justify-center">
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-32 items-center text-black font-sans">
                <div className="space-y-16">
                   ...
                   <p className="text-3xl md:text-4xl font-light italic opacity-60 leading-relaxed uppercase tracking-tight text-black font-sans">
                      We treat architecture as code. Every structure is a function of its environmental variables and tectonic intent. 100% precision. Zero noise.
                   </p>
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-12 pt-12 border-t border-black/10 font-mono text-black">
                      {[
                        { icon: <Sparkles className="w-6 h-6" />, t: "Adaptive Flow", v: "Dynamic Load Sync" },
                        { icon: <Plus className="w-6 h-6" />, t: "Structural Sync", v: "Deep_Material_ID" },
                      ].map((item, i) => (
                        <div key={i} className="flex gap-8 group text-black">
                           ...
                           <div className="text-left text-black">
                              ...
                              <p className="text-[10px] opacity-30 uppercase tracking-[0.3em] font-black leading-relaxed text-black font-mono">{item.v}</p>
                           </div>
                        </div>
                      ))}
                   </div>
      ...
      <footer className="fixed bottom-0 left-0 w-full p-8 md:p-12 z-50 flex justify-between items-end mix-blend-difference pointer-events-none opacity-20 text-[8px] uppercase font-black tracking-[0.5em] italic text-black leading-none font-mono">
         <div className="flex gap-12 text-black">
            <span>Void_OS_Alpha</span>
            <span>Uptime: 99.9%</span>
         </div>
         <div className="flex gap-4 items-end text-black font-mono">
            <div className="text-right leading-tight italic">
               Archival_Control <br /> v4.0.147
            </div>
