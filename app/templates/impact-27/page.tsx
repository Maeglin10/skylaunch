...
const INTERFACES = [
  { 
    ...
    desc: "A high-fidelity minimalist shell designed for the sub-millisecond orchestration of neural weights."
  },
  { 
    id: "UI_02",
    title: "QUANTUM_GRID", 
    ...
    desc: "Planetary-scale distributed grid for the immutable visualization of global training manifests."
  },
  { 
    id: "UI_03",
    title: "VOID_CANVAS", 
    ...
    desc: "A zero-latency creative engine built for the real-time synthesis of non-standard visual objects."
  }
];

const METRICS = [
  { label: "Sync_Rate", val: "120 FPS", desc: "Absolute UI synchronization across all distributed creative edge nodes." },
  { label: "Latency", val: "0.2ms", desc: "Sustainable packet delivery through our dedicated interface backbone." },
  { label: "Reliability", val: "99.99%", desc: "Zero-leak architectural logic verified through continuous stress-testing." }
];

const CAPABILITIES = [
  { icon: Layout, title: "Spatial Grid", desc: "Engineering interface volumes through a lens of mathematical purity." },
  { icon: MousePointer2, title: "Kinetic UI", desc: "Scaling user interactions through distributed gesture orchestration." },
  { icon: Cpu, title: "Core Logic", desc: "Synchronizing interface spikes with real-time biological demand cycles." },
  { icon: Eye, title: "Total View", desc: "Monitoring every pixel-level transaction with absolute architectural clarity." }
];
...
export default function PerspectiveOSSPA() {
  ...
  const layerY = useTransform(scrollY, [0, 1000], [0, -100]);
    ...
    <div className="min-h-screen bg-[#050505] text-[#eee] font-mono selection:bg-[#eee] selection:text-black">
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <span className="text-2xl font-black tracking-tighter uppercase italic text-white">PERSPECTIVE<span className="text-white/30">//</span>OS</span>
        </div>
        
        <div className="hidden lg:flex items-center gap-16 text-[10px] font-bold uppercase tracking-[0.4em] text-white/40">
          {["Manifest", "Interface", "Logic", "Portal"].map(item => (
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div 
            ...
            className="fixed inset-0 z-[60] bg-[#050505] text-[#eee] p-12 flex flex-col justify-between"
          >
            <div className="flex justify-between items-center border-b border-white/10 pb-12">
              <span className="text-xl font-black uppercase tracking-tighter italic">PERSPECTIVE//OS</span>
            ...
            <div className="flex flex-col gap-10">
              {["INTERFACE_MANIFEST", "TELEMETRY_ARCHIVE", "LOGIC_SHELL", "CORE_TOPOLOGY", "SECURE_AUTH"].map((item, i) => (
                <motion.a 
                  key={item}
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6">
          <Reveal>
             <div className="flex items-center justify-center gap-4 mb-12">
               ...
               <span className="text-[10px] font-bold uppercase tracking-[1.5em] text-white/40">High_Fidelity_Interface_Lab</span>
               <span className="w-12 h-[1px] bg-white/20" />
             </div>
          </Reveal>
          <Reveal delay={0.2}>
            <h1 className="text-8xl md:text-[16rem] font-black tracking-tighter leading-[0.75] uppercase italic text-white mb-20">
              DEEP <br/> <span className="not-italic text-white/10">UI.</span>
            </h1>
          </Reveal>
          <Reveal delay={0.4}>
            <div className="max-w-2xl mx-auto flex flex-col items-center gap-16 border-t border-white/10 pt-20">
              <p className="text-white/40 text-xl leading-relaxed font-light uppercase tracking-[0.2em] italic leading-loose text-center">
                Engineering the ultimate creative environments through distributed interface orchestration. High-fidelity systems built for absolute spatial precision.
              </p>
        ...
        <div className="absolute bottom-12 left-12 right-12 flex justify-between items-end text-[10px] font-bold uppercase tracking-[0.5em] text-white/20">
          <div className="flex flex-col gap-2">
            <span>INTERFACE_v9</span>
            <div className="w-48 h-[1px] bg-white/10" />
          </div>
          <div className="flex items-center gap-4 italic tracking-widest">
             <div className="w-2 h-2 bg-white rounded-full animate-pulse" /> OS_STATUS: NOMINAL
          </div>
        </div>
      </section>
          ...
          <Reveal className="mb-32">
             <div className="flex flex-col lg:flex-row justify-between items-end gap-12 border-b border-white/10 pb-12">
               <h2 className="text-7xl md:text-[10rem] font-black italic tracking-tighter leading-[0.8] uppercase text-white">
                 UI <br/> <span className="text-white/20 not-italic">Archive.</span>
               </h2>
                  ...
                  <div className="flex gap-4">
                    {INTERFACES.map((_, i) => (
                      <button 
                        ...
                        className={`w-16 h-1 transition-all ${activeUI === i ? "bg-white w-32" : "bg-white/10"}`}
                      />
                    ))}
                  </div>
               </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-24 items-center">
            <div className="lg:col-span-8 relative aspect-video rounded-sm overflow-hidden border border-white/5 group bg-[#080808]">
              <AnimatePresence mode="wait">
                ...
                >
                  <Image src={INTERFACES[activeUI].img} alt={INTERFACES[activeUI].title} fill className="object-cover grayscale contrast-125 opacity-40 group-hover:opacity-60 transition-opacity duration-1000" unoptimized />
                  <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-90" />
                </motion.div>
              </AnimatePresence>
              <div className="absolute bottom-12 left-12 flex flex-col gap-2">
                 <span className="text-[10px] font-bold uppercase tracking-widest text-white/40">CORE_VERSION</span>
                 <span className="text-xl font-black text-white italic">{INTERFACES[activeUI].version}</span>
              </div>
            </div>
            ...
            <div className="lg:col-span-4 space-y-12">
               ...
               >
                 <span className="text-[10px] font-black uppercase tracking-[0.4em] text-white/40">{INTERFACES[activeUI].id} // UI_DATA</span>
                 <h3 className="text-6xl font-black italic uppercase text-white tracking-tighter">{INTERFACES[activeUI].title}</h3>
                 <div className="space-y-6 border-y border-white/10 py-12">
                    <div className="flex justify-between items-center">
                       ...
                       <span className="text-sm font-black text-white uppercase tracking-widest">{INTERFACES[activeUI].category}</span>
                    </div>
                    <div className="flex justify-between items-center">
        ...
        <div className="max-w-[1600px] mx-auto px-8 md:px-16 grid grid-cols-1 lg:grid-cols-2 gap-32 items-center">
          <Reveal>
             <div className="relative aspect-square bg-[#080808] border border-white/10 p-12 flex flex-col justify-between overflow-hidden group perspective-[1000px]">
                ...
                <motion.div 
                  style={{ y: layerY, rotateX: 20 }}
                  className="space-y-8"
                >
                   <div className="w-full h-32 bg-white/5 border border-white/10 rounded-sm" />
                   <div className="w-[80%] h-32 bg-white/5 border border-white/10 rounded-sm translate-x-12" />
                   <div className="w-[60%] h-32 bg-white/5 border border-white/10 rounded-sm translate-x-24" />
                </motion.div>
                <div className="space-y-8 relative z-10">
                   <h3 className="text-5xl font-black italic uppercase text-white">System <br/> <span className="text-white/20 not-italic">Layering.</span></h3>
                   <p className="text-white/30 text-lg leading-relaxed mb-12 font-light uppercase tracking-wide italic leading-loose">
                     A unified shell environment for the high-fidelity orchestration of planetary-scale creative enclaves. Built for those who monitor the systems.
                   </p>
                </div>
             </div>
          </Reveal>
          <div className="space-y-24">
             <Reveal delay={0.2}>
                ...
                <h2 className="text-6xl md:text-8xl font-black italic tracking-tighter leading-none uppercase text-white">Interface <br/> <span className="text-white/20 not-italic">Synthesis.</span></h2>
             </Reveal>
             <div className="space-y-12">
                {[
                  { n: "01", t: "Depth Synapse", d: "Connecting disparate interface nodes into a unified reactive environment." },
                  { n: "02", t: "Entropy Zero", d: "Aggressive reduction of system noise for absolute visual clarity." },
            ...
            <div className="bg-white text-black p-24 lg:p-40 relative overflow-hidden flex flex-col items-center text-center group">
               ...
               <Reveal>
                  ...
                  <h2 className="text-7xl md:text-[12rem] font-black italic tracking-tighter leading-[0.8] uppercase mb-16">
                     Initiate <br/> <span className="text-black/30 not-italic">The OS.</span>
                  </h2>
      ...
      <footer className="bg-black pt-40 pb-20 px-8 md:px-16 border-t border-white/10">
         <div className="max-w-[1600px] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-32 mb-40">
            <div className="lg:col-span-6">
               <div className="flex items-center gap-4 mb-12">
                 <Layers className="w-10 h-10 text-white" />
                 <span className="text-3xl font-black tracking-tighter uppercase italic text-white">PERSPECTIVE<span className="text-white/30">//</span>OS</span>
               </div>
               <p className="text-white/40 text-sm font-light leading-relaxed uppercase tracking-[0.3em] mb-12 italic max-w-md">
                 Securing the future of distributed interfaces through high-fidelity orchestration and radical visual clarity.
               </p>
            ...
            <div className="lg:col-span-2">
               ...
               <ul className="space-y-6 text-xs font-bold uppercase tracking-[0.4em]">
                 {["Interfaces", "Telemetry", "Shell", "Journal"].map(item => (
            ...
            <div className="lg:col-span-4">
               ...
               <p className="text-sm text-white/40 font-light mb-12 italic uppercase tracking-[0.2em] leading-loose">
                 For high-priority enclaves, interface integrations, or architectural consultations, contact our core command center.
               </p>
               <a href="mailto:ops@perspective-os.io" className="text-3xl font-black italic hover:text-white transition-colors block border-b border-white/10 pb-8 uppercase tracking-tighter">
                  ops@perspective-os.io
               </a>
            </div>
         </div>
         ...
         <div className="max-w-[1600px] mx-auto flex flex-col md:row items-center justify-between gap-12 text-[9px] font-bold uppercase tracking-[0.8em] text-white/20 border-t border-white/5 pt-20">
            <p>© 2024 PERSPECTIVE OS SYSTEMS. ALL RIGHTS RESERVED. GLOBAL // SYNC.</p>
