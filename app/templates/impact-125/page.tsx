...
const AURAS = [
  { id: 1, title: "INTENT_CORE", cat: "Resonance", value: "Verified", img: "https://images.unsplash.com/photo-1485470733090-0aae1788d668?q=80&w=1000&auto=format&fit=crop" },
  { id: 2, title: "PULSE_NODE", cat: "Frequency", value: "Active", img: "https://images.unsplash.com/photo-1549490349-8643362247b5?q=80&w=1000&auto=format&fit=crop" },
  { id: 3, title: "VOID_SHELL", cat: "Singularity", value: "Locked", img: "https://images.unsplash.com/photo-1550684848-fac1c5b4e853?q=80&w=1000&auto=format&fit=crop" },
];
...
function AuraCursor() {
  ...
  return (
    <motion.div 
      ...
      className="fixed top-0 left-0 w-32 h-32 -translate-x-1/2 -translate-y-1/2 bg-amber-400 rounded-full mix-blend-difference pointer-events-none z-[100] opacity-50 blur-xl"
    />
  );
}

export default function AuraInteractiveSPA() {
  const [view, setView] = useState<"aura" | "portal" | "logic">("aura");
  const [activeItem, setActiveItem] = useState(0);

  return (
    <div className="premium-theme bg-[#0f0f0f] text-[#fbbf24] min-h-screen selection:bg-amber-600 selection:text-black font-sans overflow-x-hidden cursor-none">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        ...
        <div className="absolute inset-0 bg-[#0f0f0f]/40 backdrop-blur-[2px]" />
        <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-[0.05] mix-blend-screen" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_transparent_0%,_#0f0f0f_100%)] opacity-80" />
      </div>

      {/* Editorial HUD Nav */}
      <nav className="fixed top-0 left-0 w-full z-50 p-8 md:p-12 flex justify-between items-center bg-black/40 backdrop-blur-3xl border-b border-amber-500/10 font-mono">
        <div className="flex gap-12 items-center">
           <button onClick={() => setView("aura")} className="text-xl font-black italic tracking-tighter hover:text-white transition-colors flex items-center gap-4 text-amber-500">
              AURA_OS&trade;
           </button>
           <div className="hidden lg:flex gap-8 text-[10px] font-black uppercase tracking-widest opacity-20 italic text-white">
              ...
              <span className="text-white">Ref: 0x125</span>
           </div>
        </div>
        <div className="hidden md:flex gap-12 text-[10px] font-black uppercase tracking-[0.4em] opacity-30 text-white">
           <button onClick={() => setView("aura")} className={`hover:opacity-100 transition-opacity ${view === 'aura' ? 'text-white opacity-100 underline decoration-white underline-offset-8 italic' : ''}`}>THE_AURA</button>
      ...
      <AnimatePresence mode="wait">
        ...
        {/* THE AURA VIEW (LANDING) */}
        {view === "aura" && (
          <motion.div key="aura" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pt-48 pb-32 px-12 max-w-[1800px] mx-auto min-h-screen flex flex-col justify-center relative z-10">
             <header className="mb-24 border-b border-amber-500/20 pb-12 flex flex-col md:flex-row justify-between items-end gap-12 text-white">
                <div>
                   <span className="text-[10px] uppercase font-black tracking-[1em] opacity-40 mb-4 block underline decoration-amber-500/10 underline-offset-8 italic font-mono text-amber-500">Resonance_Capture // Series_125</span>
                   <h1 className="text-7xl md:text-[12vw] font-black italic uppercase tracking-tighter leading-[0.75]">MOVE. <br/> <span className="bg-clip-text text-transparent bg-gradient-to-r from-amber-500 to-white">INTENT.</span></h1>
                </div>
                <div className="text-right flex flex-col items-end">
                   <div className="text-3xl font-black mb-4 tracking-tighter uppercase opacity-10 italic font-mono">Singularity_Sync</div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {AURAS.map((p, i) => (
                  <motion.div 
                    ...
                    onClick={() => { setActiveItem(i); setView("portal"); }}
                  >
                     ...
                     <div className="absolute inset-10 flex flex-col justify-between">
                        <div className="flex justify-between items-start">
                           ...
                           <div className="text-[10px] font-black uppercase tracking-widest opacity-20 italic text-amber-400">NODE_0x{i+125}</div>
                        </div>
                        <div>
        ...
        {/* THE PORTAL VIEW (DETAIL) */}
        {view === "portal" && (
          <motion.div key="portal" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative z-10 min-h-screen">
             <button onClick={() => setView("aura")} className="fixed top-12 left-12 z-[60] bg-white text-black p-5 rounded-none hover:scale-110 transition-transform shadow-2xl">
                <X className="w-6 h-6" />
             </button>

             <div className="grid grid-cols-1 lg:grid-cols-12 min-h-screen pt-24 lg:pt-0">
                <div className="lg:col-span-12 relative flex items-center justify-center p-8 md:p-32 overflow-hidden h-screen bg-[#0f0f0f]">
                   <div className="absolute inset-0 opacity-10">
                      <Image src={AURAS[activeItem].img} alt="Background" fill className="object-cover grayscale" />
                   </div>
                   ...
                   <div className="max-w-[1500px] w-full grid grid-cols-1 lg:grid-cols-2 gap-24 items-center relative z-10 font-sans text-white">
                      <motion.div initial={{ scale: 1.1, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 1 }} className="relative aspect-square w-full rounded-none overflow-hidden border border-amber-500/20 group bg-neutral-900 shadow-2xl">
                         <Image src={AURAS[activeItem].img} alt="Spec" fill className="object-cover grayscale group-hover:grayscale-0 transition-all duration-[3s] opacity-80" priority />
                      ...
                      <div className="flex flex-col justify-center space-y-12">
                         <div className="space-y-6">
                            <span className="text-[10px] uppercase tracking-[1em] font-black opacity-30 mb-8 block underline decoration-white decoration-4 underline-offset-8 italic text-amber-400 font-mono">Aura_Sync // {AURAS[activeItem].cat}</span>
                            <h1 className="text-7xl md:text-[8vw] font-black italic uppercase tracking-tighter leading-none text-white">{AURAS[activeItem].title}</h1>
                            <div className="text-4xl font-black italic tracking-tighter opacity-10 italic text-amber-500">State: {AURAS[activeItem].value}</div>
                         </div>

                         <p className="text-3xl font-light italic leading-relaxed uppercase tracking-tight opacity-40 text-white leading-relaxed">
                            Structural allocation for mission {AURAS[activeItem].title}. System integrity at 100%. Thermal load nominal at 32C. Every coordinate synchronized.
                         </p>
                         ...
                         <div className="flex gap-6 pt-8 font-mono">
                            <button onClick={() => setView("aura")} className="flex-grow py-8 bg-amber-600 text-white font-black uppercase text-xs tracking-[1em] hover:bg-amber-500 transition-all shadow-2xl">
                               Return_to_Aura
                            </button>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-12 pt-12 border-t border-white/10 font-mono text-amber-400">
                      {[
                        ...
                        <div key={i} className="flex gap-8 group">
                           <div className="w-16 h-16 rounded-none border border-amber-500 flex items-center justify-center text-amber-500 group-hover:bg-amber-400 group-hover:text-black transition-all shadow-xl">
                              {item.icon}
                           </div>
                           <div className="text-left">
      ...
      {/* Global Status HUD */}
      <footer className="fixed bottom-0 left-0 w-full p-8 md:p-12 z-50 flex justify-between items-end mix-blend-difference pointer-events-none opacity-20 text-[8px] uppercase font-black tracking-[0.5em] italic text-amber-400 leading-none font-mono">
         <div className="flex gap-12 text-amber-400">
            <span>Aura_OS_Alpha</span>
            <span>Uptime: 99.9%</span>
         </div>
         <div className="flex gap-4 items-end text-amber-400">
            <div className="text-right leading-tight italic">
               Archival_Control <br /> v4.0.125
            </div>
