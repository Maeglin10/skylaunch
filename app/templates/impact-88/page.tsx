...
import { X, Menu, Search, MousePointer2, Zap, Activity, Globe, Shield, Command, Plus, ArrowUpRight, Maximize2, MoveRight, Layers, Box, Compass, Sparkles, Magnet } from "lucide-react";
import "../premium.css";

const NODES = [
  { id: 1, title: "INCEPTION", cat: "Core", value: "Locked", img: "https://images.unsplash.com/photo-1550751827-4bd374c3f58b?q=80&w=1000&auto=format&fit=crop" },
  { id: 2, title: "STRUCTURE", cat: "Logic", value: "Active", img: "https://images.unsplash.com/photo-1518770660439-4636190af475?q=80&w=1000&auto=format&fit=crop" },
  { id: 3, title: "SYNERGY", cat: "Phase", value: "Verified", img: "https://images.unsplash.com/photo-1451187580459-43490279c0fa?q=80&w=1000&auto=format&fit=crop" },
  { id: 4, title: "VOID_GATE", cat: "Security", value: "Prime", img: "https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?q=80&w=1000&auto=format&fit=crop" },
];

export default function MagneticResonanceSPA() {
  const [view, setView] = useState<"grid" | "node" | "field">("grid");
  const [activeItem, setActiveItem] = useState(0);

  return (
    <div className="premium-theme bg-[#0a0a1a] text-white min-h-screen selection:bg-white selection:text-black font-mono overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 w-full z-50 p-8 md:p-12 flex justify-between items-center bg-black/40 backdrop-blur-3xl border-b border-white/10">
        <div className="flex gap-12 items-center">
           <button onClick={() => setView("grid")} className="text-xl font-black italic tracking-tighter hover:text-white transition-colors flex items-center gap-4">
              <Magnet className="w-6 h-6 animate-pulse" /> MAGNETIC_OS&trade;
           </button>
           <div className="hidden lg:flex gap-8 text-[10px] font-black uppercase tracking-widest opacity-20 italic">
              Status: Field_Distribution_Active
              <span className="text-white">Ref: 0x88_M</span>
           </div>
        </div>
        <div className="hidden md:flex gap-12 text-[10px] font-black uppercase tracking-[0.4em] opacity-30">
           ...
           <button onClick={() => setView("field")} className={`hover:opacity-100 transition-opacity ${view === 'field' ? 'text-white opacity-100 underline decoration-white underline-offset-8 italic' : ''}`}>THE_FIELD</button>
        </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {view === "grid" && (
          <motion.div key="grid" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pt-48 pb-32 px-12 max-w-[1800px] mx-auto min-h-screen flex flex-col justify-center relative z-10">
             <header className="mb-24 border-b-2 border-white/20 pb-12 flex flex-col md:flex-row justify-between items-end gap-12">
                <div>
                   <span className="text-[10px] uppercase font-black tracking-[1em] opacity-40 mb-4 block underline decoration-white/10 underline-offset-8 italic">Field_Sync // Series_088</span>
                   <h1 className="text-7xl md:text-[12vw] font-black italic uppercase tracking-tighter leading-[0.75] text-white">THE. <br/> <span className="text-transparent" style={{ WebkitTextStroke: '2px white' }}>FIELD.</span></h1>
                </div>
        ...
        {view === "node" && (
          <motion.div key="node" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="relative z-10 min-h-screen">
             ...
             <div className="grid grid-cols-1 lg:grid-cols-12 min-h-screen pt-24 lg:pt-0">
                <div className="lg:col-span-12 relative flex items-center justify-center p-8 md:p-32 overflow-hidden h-screen bg-[#0a0a1a]">
                   ...
                   <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,_transparent_0%,_#0a0a1a_100%)]" />
                   ...
                   <div className="max-w-[1500px] w-full grid grid-cols-1 lg:grid-cols-2 gap-24 items-center relative z-10">
                      ...
                      <div className="flex flex-col justify-center space-y-12">
                         <div className="space-y-6">
                            <span className="text-[10px] uppercase tracking-[1em] font-black opacity-40 mb-8 block underline decoration-white decoration-4 underline-offset-8 italic">Field_Sync // {NODES[activeItem].cat}</span>
        ...
        {/* THE FIELD VIEW (ABOUT) */}
        {view === "field" && (
          <motion.div key="field" initial={{ opacity: 0, scale: 1.1 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="relative z-10 pt-48 pb-32 px-12 max-w-7xl mx-auto min-h-screen flex flex-col justify-center">
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-32 items-center text-white">
                <div className="space-y-16">
                   <span className="text-[10px] uppercase font-black tracking-[1.5em] opacity-30 block underline decoration-white decoration-2 underline-offset-8 italic">The_Inquiry_Protocol</span>
      ...
      <footer className="fixed bottom-0 left-0 w-full p-8 md:p-12 z-50 flex justify-between items-end mix-blend-difference pointer-events-none opacity-20 text-[8px] uppercase font-black tracking-[0.5em] italic text-white">
         <div className="flex gap-12">
            <span>Magnetic_OS_Alpha</span>
            <span>field: 1.24_T</span>
         </div>
...
function MagneticNode({ p, i, onClick }: { p: any, i: number, onClick: () => void }) {
  ...
  return (
    <motion.div
      ...
      className="group relative h-[60vh] rounded-[4rem] overflow-hidden border border-white/10 hover:border-white transition-all cursor-pointer shadow-2xl bg-white/5"
      onClick={onClick}
    >
       ...
       <div className="absolute inset-10 flex flex-col justify-between">
          ...
          <div>
             <span className="text-[10px] uppercase font-black tracking-widest opacity-40 mb-2 block italic">{p.cat}</span>
