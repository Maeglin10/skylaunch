...
import { X, Menu, Search, Box, Layers, Zap, Activity, Globe, Shield, Command, Plus, ArrowUpRight, Maximize2, Cpu, Gauge, MoveDiagonal2, Compass } from "lucide-react";
import "../premium.css";

const FEATURES = [
  { id: 1, title: "HOLOGRAPH_CORE", cat: "Hardware", value: "84%", img: "https://images.unsplash.com/photo-1550751827-4bd374c3f58b?q=80&w=1000&auto=format&fit=crop", desc: "A high-precision light-field generator for volumetric visualization." },
  { id: 2, title: "TILT_SENSOR", cat: "Interface", value: "Verified", img: "https://images.unsplash.com/photo-1518770660439-4636190af475?q=80&w=1000&auto=format&fit=crop", desc: "Sub-millisecond latency for spatial tracking and alignment." },
  { id: 3, title: "GRID_SYNC", cat: "Protocol", value: "Active", img: "https://images.unsplash.com/photo-1558494949-ef010cbdcc51?q=80&w=1000&auto=format&fit=crop", desc: "Global node synchronization across decentralized mesh networks." },
];

export default function HolographicUnitSPA() {
  const [view, setView] = useState<"unit" | "spec" | "link">("unit");
  const [activeItem, setActiveItem] = useState(0);

  return (
    <div className="premium-theme bg-[#030303] text-[#38bdf8] min-h-screen selection:bg-[#38bdf8] selection:text-black font-mono overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 w-full z-50 p-6 md:px-12 md:py-8 flex justify-between items-center bg-black/40 backdrop-blur-xl border-b border-[#38bdf8]/10">
        <div className="flex gap-12 items-center">
           <button onClick={() => setView("unit")} className="text-xl font-black italic tracking-tighter hover:text-white transition-colors flex items-center gap-4">
              <Box className="w-6 h-6 animate-pulse" /> TILT_UNIT&trade;
           </button>
           <div className="hidden lg:flex gap-8 text-[10px] font-black uppercase tracking-widest opacity-20 italic">
              ...
              <span className="text-white">Ref: 0x50_U</span>
           </div>
        </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {/* THE UNIT VIEW (HOLOGRAPHIC LANDING) */}
        {view === "unit" && (
          <motion.div key="unit" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pt-48 pb-32 px-12 max-w-[1800px] mx-auto min-h-screen flex flex-col justify-center">
             <header className="mb-24 border-b-2 border-[#38bdf8]/20 pb-12 flex flex-col md:flex-row justify-between items-end gap-12">
                <div>
                   <span className="text-[10px] uppercase font-black tracking-[1em] text-[#38bdf8] opacity-40 mb-4 block underline decoration-[#38bdf8]/10 underline-offset-8 italic">Unit_Deployment // Series_050</span>
                   <h1 className="text-7xl md:text-[12vw] font-black italic uppercase tracking-tighter leading-[0.75] text-white">THE. <br/> <span className="text-[#38bdf8]">HOLO.</span></h1>
                </div>
                <div className="text-right flex flex-col items-end">
                   <div className="text-3xl font-black mb-4 tracking-tighter uppercase opacity-10 italic">Secure_Sync</div>
                   <div className="w-64 h-1 bg-white/5 rounded-full overflow-hidden">
                      <motion.div animate={{ width: ['20%', '90%', '40%', '75%'] }} transition={{ duration: 4, repeat: Infinity }} className="h-full bg-[#38bdf8]" />
                   </div>
                </div>
             </header>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {FEATURES.map((p, i) => (
                  <motion.div 
                    ...
                    className="group relative h-[60vh] rounded-[3rem] overflow-hidden border border-[#38bdf8]/10 hover:border-[#38bdf8]/40 transition-all cursor-pointer shadow-2xl overflow-hidden"
                   ...
                   <div className="max-w-[1500px] w-full grid grid-cols-1 lg:grid-cols-2 gap-24 items-center relative z-10">
                      ...
                      <div className="flex flex-col justify-center space-y-12">
                         <div className="space-y-6">
                            <span className="text-[10px] uppercase tracking-[1em] font-black text-[#38bdf8]/40 mb-8 block underline decoration-white decoration-4 underline-offset-8 italic">Sync_Active // {FEATURES[activeItem].cat}</span>
                            <h1 className="text-7xl md:text-[10vw] font-black italic uppercase tracking-tighter leading-none text-white">{FEATURES[activeItem].title}</h1>
                         ...
                         <p className="text-3xl font-light italic leading-relaxed uppercase tracking-tight opacity-40 text-white leading-relaxed">
                            {FEATURES[activeItem].desc} Every coordinate synchronized for minimal latency and maximum throughput.
                         </p>
                         ...
                         <div className="grid grid-cols-2 gap-12 py-12 border-y border-[#38bdf8]/10 text-[#38bdf8]">
                            {[
                              ...
                              { icon: <Activity className="w-5 h-5" />, l: "Sync", v: FEATURES[activeItem].value },
                            ].map((s, i) => (
             ...
             <div className="grid grid-cols-1 lg:grid-cols-2 gap-32 items-center text-white">
                <div className="space-y-16">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-12 pt-12 border-t border-[#38bdf8]/20 text-[#38bdf8]">
                      {[
                        { icon: <MoveDiagonal2 className="w-6 h-6" />, t: "Adaptive Flow", v: "Dynamic Load Sync" },
                ...
                <div className="relative aspect-square bg-[#ddd] rounded-[4rem] p-12 overflow-hidden border border-[#38bdf8]/10 group shadow-2xl">
